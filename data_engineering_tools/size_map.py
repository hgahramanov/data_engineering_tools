...
def chain_func(x):
    return lambda a, b=x: compute_iou(a, b)

def truth_list(target_path, pred_path, iou_thresh):
    ...
    for i in range(len(small_pred)):
        temp = []
        if small_pred[i] == []:
            truth_small.append(temp)
            continue
        elif small_tar[i] == [] and small_pred[i] != []:
            temp.append(False)
        else:
            for j in range(len(small_pred[i])):
                f = compute_iou(small_pred[i][j])
                iou = max(list(map(f, small_tar[i])))
                if iou>=thresh:
                    if small_pred[0] == small_tar[0]:
                        temp.append(True)
                    else:
                        temp.append(False)
                else:
                    temp.append(False)
        turth_small.append(temp)
        

                
    ious = compute_iou(small_tar[i])
